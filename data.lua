...
local function tint_sprite(sprite, tint)
    ...
    if sprite.hr_version then tint_sprite(sprite.hr_version, tint) end
    if sprite.layers then
        for _, layer in ipairs(sprite.layers) do tint_sprite(layer, tint) end
    end
end
local function tint_icons(t, tint)
    ...
    if t.icons then for _, icon in ipairs(t.icons) do icon.tint = tint end end
end
...
local function create_prefab_data(spec)
    ...
    return {
        prefab_placed_entity, prefab_build_entity, prefab_item,
        prefab_build_item, prefab_recipe
    }
end

for _, spec in pairs(constants.prefab) do data:extend(create_prefab_data(spec)) end
...
data:extend{
    {
        ...
        effects = {
            {type = "unlock-recipe", recipe = constants.prefab["prefab"].name},
            {
                type = "unlock-recipe",
